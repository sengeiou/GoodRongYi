...
import com.lichi.goodrongyi.ui.activity.loans.AplayActivity;
 ...
 */
public class LoansFragment extends BaseFragment<LoansView, LoansPresenter> implements LoansView {
    private MyListView rlLoans;
    private WrapAdapter<LoansBean> adapter;
    private List<LoansBean> data = new ArrayList<>();
    ...
    public void setAdapter() {
        ...
        adapter = new WrapAdapter<LoansBean>(getContext(), R.layout.loans_item, data) {
            @Override
            public void convert(ViewHolder holder, final LoansBean loansBean, int position) {
    ...
    @Override
    public LoansPresenter initPresenter() {
        return new LoansPresenter();
    }

    @Override
    public void dataBillSucceed(List<LoansBean> loansBean) {
        data.clear();
    ...
    @Override
    public void datagetBannerSucceed(List<LoansBean> loansBean) {
        mimageurls.clear();
/*        for (int i = 1; i <= 5; i++) {
            mimageurls.add("http://img1.3lian.com/2015/a1/95/d/105.jpg");
        }*/
        for (LoansBean data : loansBean) {
